...
public:
    FeatureTracker();
    map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> trackImage(double _cur_time, const shared_ptr<cv::Mat> &_img, const shared_ptr<cv::Mat> &_img1 = NULL);
    void setMask();
    ...
    cv::Mat mask;
    cv::Mat fisheye_mask;
    shared_ptr<cv::Mat> prev_img, cur_img;
